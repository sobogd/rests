...
import { IPosition } from "entities/positions";
...
export const createPosition = createAsyncThunk<IPosition, IPosition, IErrorResponse>(
  "positions/create",
  async (element, { rejectWithValue }) =>
    await request(rejectWithValue, "/positions/create", "POST", { ...element, id: undefined })
);

export const updatePosition = createAsyncThunk<IPosition, IPosition, IErrorResponse>(
  "positions/update",
  async (position: IPosition, { rejectWithValue }) =>
    await request(rejectWithValue, "/positions/update", "POST", position)
);

export const removePosition = createAsyncThunk<{}, { id: string }, IErrorResponse>(
  "positions/remove",
