...
export const TablesForm: React.FC = () => {
  ...
  const handleChangeValue = (e: React.ChangeEvent<HTMLInputElement>) => {
    dispatch(tablesSlice.actions.setFormValue({ name: e.target.name, value: e.target.value }));
  };
  ...
  const handleSubmitForm = () => {
    ...
    if (isValid) {
      dispatch(tablesService[request.id ? "update" : "create"](request));
    } else {
      dispatch(tablesSlice.actions.setFormData(validatedForm));
    }
  };
  ...
  return (
    <>
      <Header
        ...
        onClickBack={() => dispatch(tablesSlice.actions.toggleIsOpenForm())}
      />
      <YouSure
        onClickYes={handleDeleteItem}
        onClickNo={() => dispatch(tablesSlice.actions.toggleIsOpenYouSure())}
      ...
      <MyForm onSubmit={handleClickEnter}>
        ...
        {!!id.value && (
          <Button
            ...
            color="error"
            variant="outlined"
            onClick={() => dispatch(tablesSlice.actions.toggleIsOpenYouSure())}
          >
            Delete
          </Button>
        )}
