...
import { IPageActive } from "./interfaces/page";
...
const App: React.FC = () => {
  ...
  const { activePage, isOpenMenu } = useAppSelector((s) => s.page);
  ...
  const toggleDrawer = () => {
    dispatch(pageSlice.actions.toggleMenu());
  };

  const handleChangePage = (page: IPageActive) => () => {
    dispatch(pageSlice.actions.toggleMenu());
    dispatch(pageSlice.actions.setActivePage(page));
  };
