import pool from "../db";
import { IOrderPositionStatus } from "../interfaces/orders-positions-statuses";
...
const tableName = "orders_positions_statuses";
...
const create = async (orderPositionStatus: IOrderPositionStatus) => {
  const client = await pool.connect();

  const createdRows = await queryBuilder.insertOne(
    client,
    tableName,
    queryBuilder.mapToDb({ ...orderPositionStatus, id: undefined }, props)
  );
  client.release();
...
const updateById = async (orderPositionStatus: IOrderPositionStatus, id: number) => {
  const client = await pool.connect();
